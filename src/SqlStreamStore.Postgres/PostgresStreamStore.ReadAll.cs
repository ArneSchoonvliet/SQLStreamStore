namespace SqlStreamStore
{
    ...
    partial class PostgresStreamStore
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var (readAllResult, transactionIdsInProgress) = await ReadAllForwards(fromPositionExclusive,
                maxCount,
                prefetch,
                correlation,
                cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            Logger.TraceFormat("{correlation} {messages} | Transactions in progress: {transactionsInProgress}",
                correlation,
                hasMessages
                    ? $"Count: {messages.Count} | {string.Join("|", messages.Select((x, i) => $"Position: {x.Position} Array index: {i}"))}"
                    : "No messages",
            ...
            {
                Logger.TraceFormat(
                    "{correlation} No messages found, no transactions in progress. We will return empty list of messages with isEnd to true",
                    correlation,
                    messages);
            ...
            for(int i = 0; i < messages.Count - 1; i++)
            {
                ...
                Logger.TraceFormat(
                    "{correlation} Gap checking. Expected position: {expectedNextPosition} | Actual position: {actualPosition}",
                    correlation,
        ...
        private async Task PollTransactions(
            Guid correlation,
            TxIdList transactionsInProgress,
            CancellationToken cancellationToken)
        {
            Logger.TraceFormat(
                "{correlation} Transactions {transactions} are in progress, so gaps might be filled, start comparing",
                correlation,
                transactionsInProgress);
            ...
            do
            {
                ...
                Logger.TraceFormat(
                    "{correlation} Transactions still pending. Query 'ReadAnyTransactionsInProgress' took: {timeTaken}ms",
                    correlation,
                    sw.ElapsedMilliseconds);

                if(_settings.GapHandlingSettings.AllowSkip
                   && totalTime > _settings.GapHandlingSettings.PossibleAllowedSkipTime)
                {
                    Logger.ErrorFormat(
                ...
                {
                    Logger.ErrorFormat(
                        "{correlation} Possible DEADLOCK! One of these transactions: {transactions} is in progress for longer then {totalTime}ms",
                        correlation,
                        transactionsInProgress,
                ...
                if(delayTime > 0)
                {
                    Logger.TraceFormat("{correlation} Delay 'PollTransactions' for {delayTime}ms",
                        correlation,
                        delayTime);
                ...
                Logger.TraceFormat(
                    "{correlation} State 'PollTransactions' | count: {count} | delayTime: {delayTime} | totalTime: {totalTime}",
                    correlation,
                    count,
                    delayTime,
                    totalTime);
        ...
        private async Task<(List<(StreamMessage message, int? maxAge)>, bool)> ReadTrustedMessages(
            long fromPositionInclusive,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var readResultToReturn = readAllResult.Where(x =>
                    x.message.Position >= fromPositionInclusive
        ...
        private async Task<bool> ReadAnyTransactionsInProgress(
            TxIdList transactionIds,
            CancellationToken cancellationToken)
        {
